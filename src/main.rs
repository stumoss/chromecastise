...
const KNOWN_FILE_EXTENSIONS: [&'static str; 11] = ["mkv", "avi", "mp4", "3gp", "mov", "mpg",
                                                   "mpeg", "qt", "wmv", "m2ts", "flv"];
...
fn main() {
    ...
    let container_format = match matches.is_present("mp4") {
        true => "mp4",
        false => "mkv",
    };
...
fn process_file(file: &Path, container_format: &str, test: bool) {
    ...
    let original_video_codec = std::str::from_utf8(&output.stdout).unwrap_or_else(|e| {
    ...
    let original_audio_codec = std::str::from_utf8(&output.stdout).unwrap_or_else(|e| {
    ...
    if output_video_codec == "copy" && output_audio_codec == "copy" {
    ...
    println!("Converted file {}: Video: {} -> {} Audio: {} -> {}",
             output_file.display(),
